...
function isValid($obj) {
    ...
    var validationOutput = function (str) {
        var tokenStack = [];
        var sexyText = "";
        var ct = 0;
    ...
    if (validationOutput[0]) {
        ...
        floater.html(validationOutput[2]);
        floater.removeClass('ww_invalid_floater'); // Remove invalid floater class
    } else {
        // is invalid, render error location
        $obj.addClass('ww_invalid');
        $obj.removeClass('ww_valid');
        floater.html(validationOutput[2] + "<BR>" + spaces(validationOutput[1]) + "<span class=ww_fail>^</span>");
        floater.addClass('ww_invalid_floater'); // Add invalid floater class
    }
}
